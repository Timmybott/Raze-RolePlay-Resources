...
function toggleAduty()
    ESX.TriggerServerCallback('mx_aduty:getValues', function(group, playerLicense)
        if Config.AdutyClothing[group] then
            ...
            if aduty then
                ESX.ShowHelpNotification('Aduty aktiviert')
            elseif not aduty then
                ESX.ShowHelpNotification('Aduty deaktiviert')
            end
            ...
            ESX.ShowHelpNotification('Keine Berechtigung!')
        end
    end)
end
...
function toggleNoclip()
    ...
    -- Synchronisiere Vanish mit Noclip
    -- Wenn noclip an ist, sorge dafür, dass vanish auch an geht (und umgekehrt)
    if (noclip and not vanish) or (not noclip and vanish) then
        toggleVanish()
    end

    if noclip then
        ESX.ShowHelpNotification('Noclip aktiviert')
    else
        ESX.ShowHelpNotification('Noclip deaktiviert')
    end
    ...
    while noclip do
        ...
        if IsDisabledControlJustPressed(0, 21) then -- schneller bei Shift
        ...
        if IsDisabledControlPressed(0, 269) then -- S
            x = x - currentNoclipSpeed * dx
            y = y - currentNoclipSpeed * dy
            z = z - currentNoclipSpeed * dz
        end
        ...
        if IsDisabledControlPressed(0, 36) then -- STRG
            z = z - currentNoclipSpeed
        end
