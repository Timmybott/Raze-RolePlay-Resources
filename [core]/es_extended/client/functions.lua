...
function ESX.Progressbar(message, length, Options)
    -- Wir ignorieren die Pr fung auf esx_progressbar und nutzen direkt ZSX
    -- Syntax: exports['ZSX_UIV2']:ProgressBar(icon, text, duration, onComplete, onCancel, canCancel, disableControls, anim, prop1, prop2)
    
    local icon = (Options and Options.icon) or "fas fa-info-circle" -- Standard-Icon falls keins in Options
    local canCancel = (Options and Options.canCancel) or false
    local disableControls = (Options and Options.disableControls) or true

    return exports['ZSX_UIV2']:ProgressBar(
        icon, 
        message, 
        length, 
        function() if Options and Options.onComplete then Options.onComplete() end end, 
        function() if Options and Options.onCancel then Options.onCancel() end end, 
        canCancel, 
        disableControls, 
        Options and Options.anim or nil, 
        Options and Options.prop1 or nil, 
        Options and Options.prop2 or nil
    )
end

function ESX.ShowNotification(message, notifyType, length)
    -- Mapping f r Header und Icons basierend auf notifyType
    local header = "BENACHRICHTIGUNG"
    ...
    -- Syntax: local serial = exports['ZSX_UIV2']:Notification(header, text, icon, duration)
    local serial = exports['ZSX_UIV2']:Notification(header, message, icon, duration)
    return serial
end
...
function ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
    -- Wir holen uns die dynamische Farbe aus deinem UI-System
    local zsxColor = exports['ZSX_UIV2']:GetColor()

    local data = {
        header = {
            text = sender,              -- Der Absender (z.B. "LSPD" oder "Bank")
            color = zsxColor,           -- Nutzt nun die einheitliche Systemfarbe
            icon = 'fas fa-info-circle' -- Icon f r Advanced Notifications
        },
        -- Formatierung: Betreff fett und Nachricht darunter
        text = "" .. subject .. "\n" .. msg
    }

    -- Trigger den ZSX Export
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    -- Verhindert Spam
    if msg == lastHelpMessage then return end
    lastHelpMessage = msg

    -- 1. Ersetzung der GTA Input-Tags durch lesbare Tasten
    local formattedMsg = msg
    if type(formattedMsg) == "string" then
        formattedMsg = formattedMsg
            -- Interaktion / Standard
            :gsub("~INPUT_CONTEXT~", "[E]")
            :gsub("~INPUT_PICKUP~", "[E]")
            :gsub("~INPUT_ENTER~", "[ENTER]")

            :gsub("~INPUT_FRONTEND_LS~", "[SHIFT]")
            :gsub("~INPUT_CHARACTER_WHEEL~", "[ALT]")

            -- Fahrzeug
            :gsub("~INPUT_VEH_EXIT~", "[F]")
            :gsub("~INPUT_VEH_HORN~", "[H]")
            :gsub("~INPUT_VEH_HEADLIGHT~", "[H]")
            :gsub("~INPUT_VEH_DUCK~", "[X]")
            :gsub("~INPUT_VEH_ACCELERATE~", "[W]")
            :gsub("~INPUT_VEH_BRAKE~", "[S]")
            :gsub("~INPUT_VEH_CIN_CAM~", "[R]")
            :gsub("~INPUT_VEH_NEXT_RADIO~", "[.]")
            :gsub("~INPUT_VEH_PREV_RADIO~", "[,]")

            -- Kampf / Waffen
            :gsub("~INPUT_ATTACK~", "[LMB]")
            :gsub("~INPUT_AIM~", "[RMB]")
            :gsub("~INPUT_RELOAD~", "[R]")
            :gsub("~INPUT_DETONATE~", "[G]")
            :gsub("~INPUT_MELEE_ATTACK_LIGHT~", "[R]")
            :gsub("~INPUT_MELEE_ATTACK_HEAVY~", "[Q]")

            -- Bewegung
            :gsub("~INPUT_JUMP~", "[LEERTASTE]")
            :gsub("~INPUT_SPRINT~", "[SHIFT]")
            :gsub("~INPUT_DUCK~", "[STRG]")
            :gsub("~INPUT_MOVE_UP_ONLY~", "[W]")
            :gsub("~INPUT_MOVE_DOWN_ONLY~", "[S]")
            :gsub("~INPUT_MOVE_LEFT_ONLY~", "[A]")
            :gsub("~INPUT_MOVE_RIGHT_ONLY~", "[D]")

            -- Men s / Sonstiges
            :gsub("~INPUT_FRONTEND_ACCEPT~", "[ENTER]")
            :gsub("~INPUT_FRONTEND_CANCEL~", "[BACKSPACE]")
            :gsub("~INPUT_FRONTEND_PAUSE~", "[ESC]")
            :gsub("~INPUT_PHONE~", "[?]")
            :gsub("~INPUT_MP_TEXT_CHAT_ALL~", "[T]")
    end

    -- 2. Dynamische Farbe holen
    local zsxColor = exports['ZSX_UIV2']:GetColor()
    ...
    -- Reset der Spam-Sperre
    SetTimeout(5000, function()
        if lastHelpMessage == msg then
            lastHelpMessage = nil
        end
    end)
