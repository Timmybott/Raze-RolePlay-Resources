...
function updateColors(hex) {
    ...
    // Alle Akzent-Variablen auf die 1:1 Export-Farbe setzen
    root.style.setProperty('--accent-main', hex);
    root.style.setProperty('--accent-dark', hex);
    root.style.setProperty('--accent-rgba', hex);
    root.style.setProperty('--accent-bright', hex);
}
...
window.addEventListener('load', (e) => {
    ...
    if (animOpts == null) {
        ...
        for (let i = 0; i < animOpts.length; i++) {
            document.getElementById(animOpts[i]).style.backgroundColor = "var(--accent-bright)";
        }
    }
