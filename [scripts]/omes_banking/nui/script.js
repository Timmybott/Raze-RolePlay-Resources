...
const BankingApp = () => {
    ...
    const openSavingsAccount = () => {
        ...
        const handleAccountTransfer = (directDirection, directAmount) => {
        const directionToUse = (typeof directDirection === 'string') ? directDirection : transferDirection;
        const amountToUse = (typeof directAmount === 'string' || typeof directAmount === 'number') ? directAmount.toString() : transferAmount;

        const numericAmount = extractNumericValue(amountToUse);
        if (!numericAmount || numericAmount <= 0) return;

        const [fromAccount, toAccount] = directionToUse.split('-to-');
    ...
    const handleQuickActionSubmit = () => {
        ...
        if (quickActionType === 'toSavings') {
            const direction = 'checking-to-savings';
            const amountStr = amount.toString();
            setTransferDirection(direction);
            setTransferAmount(amountStr);
            handleAccountTransfer(direction, amountStr);
        } else if (quickActionType === 'toChecking') {
            const direction = 'savings-to-checking';
            const amountStr = amount.toString();
            setTransferDirection(direction);
            setTransferAmount(amountStr);
            handleAccountTransfer(direction, amountStr);
