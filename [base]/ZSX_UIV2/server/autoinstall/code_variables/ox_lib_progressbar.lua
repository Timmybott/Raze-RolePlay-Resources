...
local function startProgress(data)
    ...
    if progress == false then
        exports['ZSX_UIV2']:CancelProgressBar()
        progress = nil
        return false
    end
    exports['ZSX_UIV2']:FinishProgressBar()
    return true
end
...
function lib.progressBar(data)
    ...
    if not interruptProgress(data) then
        exports['ZSX_UIV2']:ProgressBar('fas fa-circle-notch', data.label, data.duration, nil, nil, data.canCancel, nil, nil, nil, nil, true)
...
RegisterNetEvent('ZSX_UIV2:ProgressComplete', function(data, cb)
    progress = nil
end)
